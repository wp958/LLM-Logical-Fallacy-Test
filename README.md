# LLM-Logic-Eval-Framework
An automated evaluation framework for LLM logic reasoning, testing 8 types of logical fallacies using Python. 
# LLM Logic Evaluation Framework (大模型逻辑推理评测框架)

## 📖 Project Introduction (项目简介)
This project is an automated framework designed to evaluate the logical reasoning capabilities of Large Language Models (LLMs).
(本项目是一个针对大语言模型逻辑推理能力的自动化评测框架。)

It focuses on resisting logical fallacies, including **Curry's Paradox (库里悖论)**, **Gambler's Fallacy (赌徒谬误)**, and **Quantifier Ambiguity (量词歧义)**.

## 🚀 Key Features (核心功能)
- **Logic Probes:** 8 types of logical traps designed based on Formal Logic.
- **Auto-Evaluation:** Python scripts utilizing Spark API for batch testing.
- **Metric Visualization:** Confusion matrix analysis and confidence score distribution.

## 🛠️ Tech Stack (技术栈)
- **Language:** Python 3.9
- **Libraries:** Pandas, NetworkX, Matplotlib, Jieba, Requests
- **Model:** Spark X1 (via API)

## 📊 Results (结果展示)
> Here you can upload your charts!
<img width="3000" height="1800" alt="accuracy_by_module" src="https://github.com/user-attachments/assets/6cd74b70-0082-4169-a42d-eb859766c6ff" />
<img width="3600" height="2400" alt="fallacy_confusion_matrix" src="https://github.com/user-attachments/assets/6bd50a6f-36b7-4a97-88d1-bfa3fc42a0bd" />
<img width="3000" height="1800" alt="confidence_distribution" src="https://github.com/user-attachments/assets/35c9e5c2-4ad1-42f7-bdda-ad54e90f57b7" />

## 👨‍💻 Author
**Wang Pan**
Philosophy Student @ Xiangtan University
Focusing on AI & Logic.
